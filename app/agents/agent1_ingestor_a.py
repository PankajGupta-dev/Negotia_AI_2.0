    ...
    def _execute(
        ...
        **kwargs: Any,
        ...
        if settings.GEMINI_API_KEY:
            try:
                self.emit_event(
                    thought="Invoking Google Gemini LLM for forensic position mapping...",
                    matter_id=matter_id,
                )
                return self._analyze_with_gemini(clauses, matter_id)
            except Exception as err:
                logger.warning(f"Gemini API call failed, falling back to deterministic parser: {err}")
        elif settings.OPENAI_API_KEY:
            try:
                self.emit_event(
                    thought="Invoking OpenAI LLM for forensic position mapping...",
                    matter_id=matter_id,
                )
                return self._analyze_with_openai(clauses, matter_id)
            except Exception as err:
                logger.warning(f"OpenAI API call failed, falling back to deterministic parser: {err}")

        # 3. Deterministic Fallback Parser
        self.emit_event(
            thought="Mapping Party A legal parameters using deterministic baseline classifier...",
            matter_id=matter_id,
        )
