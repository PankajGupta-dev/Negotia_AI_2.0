        ...
        **kwargs: Any,
        ...
        if settings.GEMINI_API_KEY:
            try:
                ...
                logger.info("[AGENT 2] Mode: LLM | Attempting semantic deviation analysis via Gemini.")
                result = self._analyze_with_gemini(
                    party_a_clauses, party_b_clauses, diffs, a_map, b_map, matter_id,
                )
                logger.info("[AGENT 2] Mode: LLM | Successfully enriched risk profiles via Gemini with deterministic validation.")
                return result
            except Exception as err:
                logger.warning(
                    f"[AGENT 2] Gemini API call failed: {err}. Falling back to deterministic risk rules."
                )
        elif settings.OPENAI_API_KEY:
            try:
                self.emit_event(
                    thought="[Mode: LLM] Invoking OpenAI for semantic deviation analysis...",
                    matter_id=matter_id,
                )
                logger.info("[AGENT 2] Mode: LLM | Attempting semantic deviation analysis via OpenAI.")
                result = self._analyze_with_openai(
                    party_a_clauses, party_b_clauses, diffs, a_map, b_map, matter_id,
                )
                logger.info("[AGENT 2] Mode: LLM | Successfully enriched risk profiles via OpenAI with deterministic validation.")
                return result
            except Exception as err:
                logger.warning(
                    f"[AGENT 2] OpenAI API call failed: {err}. Falling back to deterministic risk rules."
                )

        # ── Deterministic Fallback ───────────────────────────────────────
        logger.info("[AGENT 2] Mode: FALLBACK | Using deterministic risk rules & keyword heuristics.")
        self.emit_event(
            thought="[Mode: FALLBACK] Executing deterministic redline risk analysis pipeline...",
            matter_id=matter_id,
        )
