...
class AuditService:
    ...
    def verify_audit_chain(db: Session, matter_id: str) -> Dict[str, Any]:
        ...
        # Topologically order blocks by following cryptographic hash links starting from GENESIS_HASH
        records_by_prev = {r.previous_hash: r for r in records}
        ordered_records = []
        curr_prev = GENESIS_HASH
        while curr_prev in records_by_prev:
            r = records_by_prev[curr_prev]
            ordered_records.append(r)
            curr_prev = r.sha256_hash
